...
> **AIに頼んだあと、何が起きるか見えていますか。**
>
> 整えるのはUME。決めるのは人。

日本語で、雑に頼める。「いい感じに直して」と頼みたい。でも、どこまで変わるのかは確かめたい。
UME-HARNESSは、そんな依頼を作業案へ整理する、日本語を中心に設計したハーネスです。

作業へ進む前に、preview CLIで「確認できる範囲」と「確認が必要な操作」を見える形にします。
standalone CLIはファイル操作を実行しません。

Claude Codeとの標準接続では、権限確認やツール実行失敗に日本語の補足説明を添えます。
権限判断はClaude Codeの既存設定に従い、UMEによる追加の実行制限は行いません。
追加制限を使う場合は、明示的に `--managed` を選びます。
